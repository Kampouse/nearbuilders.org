...
export async function migrate(db: NotificationsDatabase, migrations: Migration[]): Promise<void> {
  ...
  for (const migration of migrations) {
    ...
    console.log(`[Notifications] Applying migration: ${migration.tag}`);
    ...
    for (const statement of migration.sql) {
      try {
        ...
        if (IDEMPOTENT_ERRORS.test(msg)) {
          console.log(
            `[Notifications] Skipping statement (already applied): ${statement.slice(0, 80)}`,
          );
          continue;
        }
        throw err;
